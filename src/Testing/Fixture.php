...
final class Fixture
{
    ...
    /**
     * Sensitive JSON parameters to redact.
     *
     * @var array<string, callable|string>
     */
    private array $sensitiveJsonParameters = [];
    ...
    /**
     * Clear the recorder callback.
     */
    public static function clearRecorder(): void
    {
        self::$recorder = null;
    }
    ...
    {
        $this->sensitiveJsonParameters = $parameters;
    ...
    {
        $this->sensitiveRegexPatterns = $patterns;
    ...
    private function record(): Response
    {
        ...
        // Make the real request using the recorder
        $response = (self::$recorder)($this->request);

        // Store the response to the fixture file
        $this->store($response);
    ...
    private function redactHeaders(array $headers): array
    {
        foreach (array_keys($headers) as $name) {
            foreach ($this->sensitiveHeaders as $sensitiveHeader => $replacement) {
    ...
    {
        return $this->redactNestedParameters($data, $this->sensitiveJsonParameters);
    }
    ...
    {
        if ($this->sensitiveRegexPatterns === []) {
            return $data;
        }
        ...
        if (is_array($data)) {
            foreach ($data as $key => $value) {
                if (is_string($value)) {
                    ...
                    foreach ($this->sensitiveRegexPatterns as $pattern => $replacement) {
