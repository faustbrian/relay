...
abstract class AbstractConnector
{
     ...
     *
     * @param array<Closure(AbstractRequest): Response|Response> $responses
     */
     ...
     */
    public function rateLimitStore(): RateLimitStoreInterface
    {
    ...
    public function send(AbstractRequest $request): Response
    {
        ...
        if ($mockClient instanceof MockClient) {
            ...
            $mockClient->setRecorder(fn (AbstractRequest $req): Response => $this->sendRaw($req));
        ...
        // Debug request if enabled (check both connector, request, and global)
        $debuggingState = $this->getActiveDebuggingState($request);

        if ($debuggingState->shouldDebugRequest()) {
            $debuggingState->outputRequest($request, $this->resolveBaseUrl(), $psrRequest);
        }
        ...
        $psrResponse = $this->getHttpClient()->sendRequest($psrRequest);
        ...
        if ($debuggingState->shouldDebugResponse()) {
            $debuggingState->outputResponse($response, $psrResponse);
        }

        // Global response debugging
    ...
    {
        return $this->send(
            new class($endpoint, $query) extends AbstractRequest
            {
                /**
                 * @param array<string, mixed> $query
                 */
    ...
    {
        return $this->send(
            new class($endpoint, $data) extends AbstractRequest
            {
                /**
                 * @param array<string, mixed> $data
                 */
    ...
    {
        return $this->send(
            new class($endpoint) extends AbstractRequest
            {
                public function __construct(
                    private readonly string $endpoint,
                ) {}
     ...
     */
    public function forgetCache(AbstractRequest $request): bool
    {
        $cache = $this->getRequestCache();
     ...
     */
    public function paginate(AbstractRequest $request, ?PaginatorInterface $paginator = null): PaginatedResponse
    {
    ...
    /**
     * Send an already-prepared request directly (bypasses mock client).
     *
     * Used for fixture recording - the request has already been initialized and authenticated.
     */
    protected function sendRaw(AbstractRequest $request): Response
    {
        // Build the full URL
        $url = $this->buildUrl($request);
        ...
        // Create our response wrapper
        return new Response($psrResponse, $request);
    }
     ...
     */
    protected function buildUrl(AbstractRequest $request): string
    {
     ...
     * @return array<string, string>
     */
    protected function mergeHeaders(AbstractRequest $request): array
    {
        $headers = [
            ...$this->defaultHeaders(),
            ...$request->allHeaders(),
     ...
     * @return array<string, mixed>
     */
    protected function buildOptions(AbstractRequest $request, array $headers): array
    {
        return [
            'headers' => $headers,
            'timeout' => $this->timeout(),
     ...
     */
    protected function buildBody(AbstractRequest $request): ?string
    {
     ...
     */
    protected function maybeThrow(AbstractRequest $request, Response $response): void
    {
     ...
     */
    protected function createClientException(AbstractRequest $request, Response $response): AbstractClientException
    {
     ...
     */
    protected function createServerException(AbstractRequest $request, Response $response): AbstractServerException
    {
        return match ($response->status()) {
     ...
     */
    protected function createPaginatorForRequest(AbstractRequest $request): PaginatorInterface
    {
    ...
    /**
     * Get the active debugging state (request-level takes precedence over connector-level).
     */
    protected function getActiveDebuggingState(AbstractRequest $request): DebuggingState
    {
        // AbstractRequest-level debugging takes precedence
        if ($request->hasDebugging()) {
            return $request->getDebuggingState();
        }

        return $this->getDebuggingState();
    }
}
