...
abstract class AbstractRequest
{
     ...
     *
     * @internal Called by AbstractResource when sending requests
     */
    public function setResource(AbstractResource $resource): static
    {
     ...
     *
     * @internal Called by AbstractConnector when sending requests
     */
    public function setConnector(AbstractConnector $connector): static
    {
    ...
    public function method(): string
    {
        ...
        if (count($foundMethods) > 1) {
            throw AttributeConflictException::multipleHttpMethods(
                static::class,
                array_map(fn (HttpMethodInterface $m): string => $m->method(), $foundMethods),
            );
        }
     ...
     * Request (including parent classes) -> AbstractResource -> AbstractConnector
     */
    public function contentType(): ?string
    {
        ...
        // Check AbstractResource if available
        ...
        // Check AbstractConnector if available
    ...
    /**
     * Resolve an attribute by walking the inheritance chain: Request -> AbstractResource -> AbstractConnector.
     *
    ...
    public function resolveAttribute(string $attributeClass): ?object
    {
        ...
        // 2. Check AbstractResource if available
        if (($attr = $this->resource?->getAttribute($attributeClass)) !== null) {
            return $attr;
        }

        // 3. Check AbstractConnector if available
     ...
     * @param  array<class-string<ContentTypeInterface>> $attributeClasses
     * @return array<ContentTypeInterface>
     */
     ...
     *
     * @param array<ContentTypeInterface> $foundTypes
    ...
    private function validateAndReturnContentType(array $foundTypes, string $sourceClass): string
    {
        if (count($foundTypes) > 1) {
            throw AttributeConflictException::multipleContentTypes(
                $sourceClass,
                array_map(fn (ContentTypeInterface $t): string => $t->contentType(), $foundTypes),
            );
        }
    ...
    private function applyIdempotency(): void
    {
        ...
        // Check for IdempotencyKeyGeneratorInterface class
        if ($attr instanceof Idempotent && $attr->keyMethod !== null) {
            if (class_exists($attr->keyMethod) && is_a($attr->keyMethod, IdempotencyKeyGeneratorInterface::class, true)) {
