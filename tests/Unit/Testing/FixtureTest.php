...
describe('Fixture Redaction', function (): void {
    ...
    it('applies redactions when storing', function (): void {
        $fixture = Fixture::make('test-redact-store')
            ->withSensitiveJsonParameters([
                'secret' => '[REDACTED]',
            ]);
...
describe('Fixture Recording', function (): void {
    afterEach(function (): void {
        Fixture::clearRecorder();
    ...
    it('sets and clears recorder callback', function (): void {
        ...
        Fixture::setRecorder($recorder);
        Fixture::clearRecorder();

        // After clearing, recording should throw
        MockConfig::throwOnMissingFixtures(false);
        $fixture = Fixture::make('nonexistent');
    ...
    it('associates request with fixture via forRequest', function (): void {
        $fixture = Fixture::make('test');
        $request = createFixtureRequest('/test-endpoint');

        $result = $fixture->forRequest($request);

        expect($result)->toBe($fixture);
    });

    it('records response when fixture missing and recorder set', function (): void {
        MockConfig::throwOnMissingFixtures(false);
        ...
        $fixture = Fixture::make('recorded-test');
        $request = createFixtureRequest('/api/data');

        // Fixture doesn't exist, should record
        expect($fixture->exists())->toBeFalse();

        $response = $fixture->forRequest($request)->resolve();

        // Response should match what recorder returned
        expect($response->json('recorded'))->toBeTrue();
        expect($response->json('endpoint'))->toBe('/api/data');

        // Fixture file should now exist
        expect($fixture->exists())->toBeTrue();

        // File should contain correct data
        $contents = json_decode(file_get_contents($fixture->getFilePath()), true);
        expect($contents['status'])->toBe(200);
        expect($contents['body']['recorded'])->toBeTrue();
    });

    it('throws when recording without recorder set', function (): void {
        MockConfig::throwOnMissingFixtures(false);
    ...
    it('applies redactions when recording', function (): void {
        ...
        $fixture = Fixture::make('recorded-test')
            ->withSensitiveHeaders(['Authorization' => '[REDACTED]'])
            ->withSensitiveJsonParameters(['password' => '[HIDDEN]']);

        $request = createFixtureRequest('/secure');
        $response = $fixture->forRequest($request)->resolve();

        // Check stored file has redactions
        $contents = json_decode(file_get_contents($fixture->getFilePath()), true);
        expect($contents['headers']['Authorization'])->toBe('[REDACTED]');
        expect($contents['body']['password'])->toBe('[HIDDEN]');
    });
});
